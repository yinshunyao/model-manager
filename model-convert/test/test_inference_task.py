...
此测试文件专门测试 rest_predict_910b.py 提供的图片检测功能，包括：
1. 图片检测任务测试
2. 回调功能测试
...
def send_inference_task(api_base_url, task_data):
    """
    发送推理任务
...
def test_inference_task(api_base_url, minio_handler_instance):
    """
    测试推理任务
    ...
    """
    logger.info("=" * 60)
    logger.info("开始测试推理任务")
    logger.info("=" * 60)
    
    # 准备测试文件路径
    om_file_path = str(test_dir / "yolo11n.om")

    # 检查文件是否存在
    if not os.path.exists(om_file_path):
        logger.error(f"测试OM文件不存在: {om_file_path}")
        return False
    
    if not image_file_path or not os.path.exists(image_file_path):
        logger.error(f"测试图片文件不存在: {image_file_path}")
        return False
    
    # 上传OM文件和图片到MinIO
    onnx_file_path = str(test_dir / "yolo11n.onnx")
    onnx_object_name = f"test_{int(time.time())}_yolo11n.onnx"
    om_object_name = f"test_{int(time.time())}_yolo11n.om"
    image_object_name = f"test_{int(time.time())}_test_image.jpg"
    test_files = [
        (om_file_path, om_object_name),
        (image_file_path, image_object_name),
        (onnx_file_path, onnx_object_name)
    ]

    r1 = upload_test_files(minio_handler_instance, BUCKET_ONNX, [test_files[2]])
    r2 = upload_test_files(minio_handler_instance, BUCKET_ENGINE, [test_files[0]])
    r3 = upload_test_files(minio_handler_instance, BUCKET_SOURCE, [test_files[1]])
    
    if any([not r1, not r2, not r3]):
        logger.error("上传测试文件失败")
        return False
    
    # 准备任务数据
    task_data = {
        "task_id": int(time.time()) + 1,  # 确保任务ID不同
        "model_id": 1002,
        "platform": "Huawei",
        "model_type": "YOLO",
        "source_file": f"{image_object_name}",
        "model_file": f"{om_object_name}",
        "onnx_file": "",  # 推理任务不需要ONNX文件
        "engine_file": f"{om_object_name}"  # 提供engine_file，触发推理任务
    }
    
    # 发送推理任务
    task_id = send_inference_task(api_base_url, task_data)
    if not task_id:
        logger.error("发送推理任务失败")
        return False
    
    logger.info(f"推理任务已发送，任务ID: {task_id}")
    
    # 等待回调结果
    callback_data = wait_for_callback(str(task_data["task_id"]))
    if not callback_data:
        logger.error("等待推理任务回调超时")
        return False
    
    # 验证回调数据
    result = callback_data.get("result", "")
    target_file = callback_data.get("target_file", "")
    
    if result and target_file:
        logger.info("推理任务测试成功")
        logger.info(f"回调结果: {callback_data}")
        # 尝试解析推理结果
        try:
            result_data = json.loads(result)
            logger.info(f"推理结果详情: {result_data}")
        except:
            logger.info(f"推理结果: {result}")
        return True
    else:
        logger.error("推理任务测试失败")
...
def main():
    """主函数"""
    logger.info("开始执行华为910b平台图片检测任务测试")
    ...
    try:
        ...
        # 测试推理任务
        inference_success = test_inference_task(api_base_url, minio_handler_instance)
        
        # 输出测试结果
        logger.info("=" * 60)
        logger.info("测试结果汇总")
        logger.info("=" * 60)
        logger.info(f"推理任务: {'通过' if inference_success else '失败'}")
        
        if inference_success:
            logger.info("图片检测任务测试通过!")
            return True
        else:
            logger.error("图片检测任务测试失败!")
            return False
...
if __name__ == "__main__":
    ...
    # 推理图片
    image_file_path = test_dir / "000000000009.jpg"
